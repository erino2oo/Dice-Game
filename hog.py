...
def roll_dice(num_rolls, dice=six_sided_dice, who='Boss Hogg', ones_lose=True):
    ...
    while(counter > 0):
        this_roll = dice()
        if(ones_lose == True): #Ones Lose Rule
            if(this_roll == 1):
                ret_val = 1
                counter = 0
            else:
                ret_val = ret_val + this_roll
                counter = counter - 1
        if(ones_lose == False):
            ret_val = ret_val + this_roll
            counter = counter - 1
    print(who, ' got ', ret_val, ' after ', num_rolls, ' rolls ')
    return ret_val

def take_turn(num_rolls, opp_score, dice=six_sided_dice, who='Minion Hogg', ones_lose=True):
    ...
    assert type(opp_score) == int, 'opp_score must be an integer'
    if commentary:
        print(who, 'is going to roll', num_rolls, 'dice')
    
    """1, 0, make_test_dice(6), 'Minion Hogg', True"""

    if(num_rolls > 0):
        score = roll_dice(num_rolls, dice, who, ones_lose)
        if(int(score) % 6 == 0): #Touchdown Rule
            score = int(score) + (int(score) / 6)
            print(who, 'got a touchdown and scored', score, 'points')
            return score    
        return score
    ...
    if(num_rolls == 0): #Free Bacon Rule
        if(opp_score < 10):
            tens_digit = 0
            score = 1
            print(who, 'rolled', num_rolls, 'dice, and gets', score, 'points')
            return score
        if(opp_score > 9): #Free Bacon Rule
            tens_digit = str(opp_score)[-2]
            score = int(tens_digit) + 1
            if(int(score) % 6 == 0): # Touchdown Rule
                score = int(score) + (int(score) / 6)
                print(who, 'got a touchdown and scored', score, 'points')
                return score
...
def num_allowed_dice(score, opponent_score):
    ...
    """
    "*** YOUR CODE HERE ***"
    if (str(sum(score, opponent_score)))[-1] == '7':
        return 1
    else:
        return 10
